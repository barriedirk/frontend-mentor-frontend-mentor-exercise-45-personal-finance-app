...
async function nxCloudPrompt(key, utmContent) {
    ...
    const suffix = [hint, footer && `${footer} ${(0, ab_testing_1.nxCloudHyperlink)(utmContent)}`]
        .filter(Boolean)
        .map((t) => pc.dim(t));
    return (await (0, prompt_helpers_1.selectPrompt)({
        ...
        choices: choices.map((c) => typeof c === 'string'
            ? { value: c, label: c }
            : { value: c.value ?? c.name, label: c.message ?? c.name ?? c.value }),
        initial: choices[initial ?? 0]?.value ??
            choices[initial ?? 0]?.name,
    }));
}
