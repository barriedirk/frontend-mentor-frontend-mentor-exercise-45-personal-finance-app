...
function withAffectedOptions(yargs) {
    return withExcludeOption(yargs)
        ...
        .option('stdin', {
        describe: 'Change the way Nx is calculating the affected command by providing directly changed files from stdin, one file per line.',
        type: 'boolean',
    })
        .option('uncommitted', {
        ...
        if (args.stdin) {
            if (process.stdin.isTTY) {
                throw new Error('The --stdin option requires piped input (e.g., `git diff --name-only | nx affected --stdin`). It cannot be used when stdin is a terminal.');
            }
            const chunks = [];
            ...
            if (Array.isArray(args.files))
                ...
                files.push(args.files);
            args.files = files; // Yargs types don't reflect the coerce option
        }
    }, true);
}
    ...
    return yargs
        ...
        (args) => {
            if (!args.outputStyle &&
                ...
                choices.includes(process.env.NX_DEFAULT_OUTPUT_STYLE)) {
