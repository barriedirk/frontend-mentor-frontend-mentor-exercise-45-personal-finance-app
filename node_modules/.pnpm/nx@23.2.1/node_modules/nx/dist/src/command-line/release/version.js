...
function createAPI(overrideReleaseConfig, ignoreNxJsonConfig) {
    ...
    return async function releaseVersion(args) {
        ...
        const manifestsToExcludeFromFormatting = new Set(Array.from(releaseGraph.projectsToVersionActions.values()).flatMap((versionActions) => versionActions.excludeManifestsFromFormatting
            ? versionActions.manifestsToUpdate.map(({ manifestPath }) => manifestPath)
            : []));
