...
exports.recordInitWrite = recordInitWrite;
exports.formatInitWrites = formatInitWrites;
 ...
 */
function recordInitWrite(filePath) {
 ...
 */
async function formatInitWrites(repoRoot, 
...
command = 'nx init') {
    ...
    // keys for one file.
    const files = [
        ...new Set(recorded.map((file) => path
        ...
        (0, node_fs_1.existsSync)(path.join(repoRoot, file)));
    if (files.length === 0) {
        return;
    }
    ...
    let formatter = null;
    try {
        formatter = (0, formatters_1.detectFormatter)(repoRoot);
        if (!formatter) {
            return;
        }
        if (formatter === 'oxfmt') {
            for (const chunk of (0, chunkify_1.chunkify)(files)) {
                if (chunk.length) {
                    (0, oxfmt_1.writeWithOxfmt)(chunk, repoRoot);
                }
            }
        }
        else {
            ...
            const supported = await (0, prettier_1.filterToPrettierSupportedFiles)(files);
            for (const chunk of (0, chunkify_1.chunkify)(supported, undefined, (pattern) => (0, prettier_1.quoteForShell)(pattern).length)) {
                if (chunk.length) {
                    (0, prettier_1.writeWithPrettier)(chunk, repoRoot);
                }
            }
        }
    }
