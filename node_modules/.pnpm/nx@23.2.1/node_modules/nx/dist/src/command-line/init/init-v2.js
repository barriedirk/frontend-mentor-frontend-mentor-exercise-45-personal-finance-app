...
async function runInit(options, baseMeta) {
    ...
    const isSandboxed = (0, is_sandbox_1.isSandbox)();
    if (aiMode) {
    ...
    if (!(0, fs_1.existsSync)('package.json') &&
        ...
        !aiMode &&
        process.stdin.isTTY) {
        const setupMode = await (0, prompt_helpers_1.selectPrompt)({
            ...
            choices: [
                ...
                {
                    value: 'package.json installation (recommended for JavaScript/TypeScript projects)',
                },
            ],
        });
        if (setupMode.startsWith('package.json')) {
            ...
            (0, format_1.recordInitWrite)('package.json');
        }
        else {
    ...
    if (options.interactive && !(_isTurborepo || _isNonJs)) {
        const setupType = await (0, prompt_helpers_1.selectPrompt)({
    ...
    const selectedAgents = await (0, ai_agent_prompts_1.determineAiAgents)(options.aiAgents, options.interactive);
    if (selectedAgents && selectedAgents.length > 0) {
        try {
            ...
            if (!(0, permission_errors_1.isPermissionDenied)(e)) {
                throw e;
            }
            ...
            output_1.output.warn({
                ...
                bodyLines: (0, utils_2.agentConfigWriteBlockedLines)(e),
            });
        }
    }
    let nxCloudChoice;
...
async function detectPlugins(nxJson, packageJson, interactive, includeAngularCli) {
    ...
    const pluginsToInstall = await (0, prompt_helpers_1.multiselectPrompt)({
    ...
    const updatePackageScripts = (0, fs_1.existsSync)('package.json') &&
        (await (0, prompt_helpers_1.confirmationPrompt)({
