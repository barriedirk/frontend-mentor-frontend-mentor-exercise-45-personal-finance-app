...
const format_1 = require("../init/implementation/format");
...
async function importHandler(options) {
    ...
    if (!sourceRepository) {
        sourceRepository = await (0, prompt_helpers_1.textPrompt)({
    ...
    if (!ref) {
        ...
        ref = await (0, prompt_helpers_1.selectPrompt)({
    ...
    if (!source) {
        if (aiMode) {
            ...
            source = await (0, prompt_helpers_1.textPrompt)({
    ...
    if (!destination) {
        ...
        destination = await (0, prompt_helpers_1.textPrompt)({
    ...
    if (installed) {
        ...
        if (nxJson.plugins?.length > 0) {
            ...
            if (Object.keys(incompatiblePlugins).length > 0) {
                ...
                await (0, format_1.formatInitWrites)(workspace_root_1.workspaceRoot, 'nx import');
...
async function handleMissingWorkspacesEntry(pm, pmc, pkgPath, destinationGitClient) {
    if (!(0, package_manager_1.isWorkspacesEnabled)(pm, workspace_root_1.workspaceRoot)) {
        ...
        (0, format_1.recordInitWrite)((0, path_1.join)(workspace_root_1.workspaceRoot, pm === 'pnpm' ? 'pnpm-workspace.yaml' : 'package.json'));
        await (0, format_1.formatInitWrites)(workspace_root_1.workspaceRoot, 'nx import');
        await destinationGitClient.amendCommit();
