...
exports.cacheSectionLines = cacheSectionLines;
...
function collapseHome(path) {
    ...
    return path === home || path.startsWith(home + path_1.sep)
        ? `~${path.slice(home.length)}`
        : path;
}
function cacheSectionLines(cache) {
    const unlimited = cache.max === 0;
    const max = unlimited ? '∞' : (0, cache_1.formatCacheSize)(cache.max);
    const share = unlimited
        ? ''
        : ` (${((cache.used / cache.max) * 100).toFixed(1)}%)`;
    return [
        'Cache',
        `- Usage: ${(0, cache_1.formatCacheSize)(cache.used)} / ${max}${share}`,
...
async function getReportData() {
    ...
    let cache = (0, cache_1.dbCacheEnabled)()
        ? {
            ...
            location: cache_directory_1.cacheDir,
        }
        : null;
    return {
        pm,
