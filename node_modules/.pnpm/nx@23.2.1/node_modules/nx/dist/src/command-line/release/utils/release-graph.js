...
class ReleaseGraph {
    ...
    async init(options) {
        if (this.filters.projects?.length) {
            this.matchedProjectsForFilter = new Set((0, find_matching_projects_1.findMatchingProjects)([...this.filters.projects], options.projectGraph.nodes));
            if (this.matchedProjectsForFilter.size === 0) {
                throw new Error(`Your --projects filter "${this.filters.projects}" did not match any projects in the workspace`);
            }
        }
