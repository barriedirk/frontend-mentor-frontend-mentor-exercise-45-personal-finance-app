...
exports.NEW_MIGRATE_FLAGS = exports.NEW_MIGRATE_FLAGS_FLOOR = void 0;
exports.findNewMigrateFlag = findNewMigrateFlag;
exports.targetsExistingRun = targetsExistingRun;
exports.resolveNewMigrateFlagsRunTarget = resolveNewMigrateFlagsRunTarget;
exports.assertWorkspaceNxSupportsNewMigrateFlags = assertWorkspaceNxSupportsNewMigrateFlags;
...
const RUN_ID_FLAGS = ['--run-id', '--runId'];
/**
...
function findNewMigrateFlag(argv) {
    return findFlag(argv, exports.NEW_MIGRATE_FLAGS);
}
/**
 ...
 */
function targetsExistingRun(argv) {
 ...
 */
async function resolveNewMigrateFlagsRunTarget(options) {
    ...
    const concrete = (0, semver_1.valid)(options.cliVersionSpec);
    let resolved;
    try {
        resolved =
        ...
        logger_1.logger.verbose(`Could not resolve the nx version for '${options.cliVersionSpec}': ${e}`);
    }
    if (resolved !== undefined &&
        !(0, semver_1.lt)((0, version_utils_1.normalizeVersion)(resolved), exports.NEW_MIGRATE_FLAGS_FLOOR)) {
        return 'temp-cli';
    }
    if (resolved !== undefined && options.fromEnvOverride) {
        throw new Error(`The nx version 'nx migrate' is about to install (${resolved}) does not support ${flag}. ` +
            `This flag ships in nx ${exports.NEW_MIGRATE_FLAGS_FLOOR} or newer. ` +
            `NX_MIGRATE_CLI_VERSION is set to '${options.cliVersionSpec}'. ` +
            `Unset it or set it to nx ${exports.NEW_MIGRATE_FLAGS_FLOOR} or newer.`);
    }
    const localNxVersion = options.readLocalNxVersion();
    if (localNxVersion !== undefined &&
        (localNxVersion === options.ownNxVersion ||
            !(0, semver_1.lt)((0, version_utils_1.normalizeVersion)(localNxVersion), exports.NEW_MIGRATE_FLAGS_FLOOR))) {
    ...
    const tempSideOutcome = resolved !== undefined
        ? `resolves to ${resolved}, which does not support it`
        : `could not be resolved`;
    const localSideOutcome = localNxVersion !== undefined
    ...
    throw new Error(`${flag} requires nx ${exports.NEW_MIGRATE_FLAGS_FLOOR} or newer. ` +
        `'${options.cliVersionSpec}' (the version 'nx migrate' would install) ${tempSideOutcome}, ` +
        `and ${localSideOutcome}. ` +
        `Update the workspace to nx ${exports.NEW_MIGRATE_FLAGS_FLOOR} or newer first, ` +
...
function assertWorkspaceNxSupportsNewMigrateFlags(options) {
    ...
    if ((0, semver_1.lt)((0, version_utils_1.normalizeVersion)(localNxVersion), exports.NEW_MIGRATE_FLAGS_FLOOR)) {
        ...
        throw new Error(`The workspace's installed nx (${localNxVersion}) does not support ${flag}. ` +
            `Update the workspace to nx ${exports.NEW_MIGRATE_FLAGS_FLOOR} or newer first, ` +
            `for example by running 'nx migrate nx@latest' (without ${flag}) or by updating your dependencies.`);
    }
}
