...
signal) {
    ...
    const installationSupportsGitHub = await getInstallationSupportsGitHub(apiUrl, signal);
...
async function getInstallationSupportsGitHub(apiUrl, signal) {
    try {
        const response = await require('axios').get(`${apiUrl}/nx-cloud/system/features`, { signal });
        if (!response?.data || response.data.message) {
            throw new Error(response?.data?.message ?? 'Failed to shorten Nx Cloud URL');
        }
