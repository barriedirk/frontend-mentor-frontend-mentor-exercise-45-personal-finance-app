...
exports.isStaticOutputStyle = isStaticOutputStyle;
exports.printsFullTaskOutput = printsFullTaskOutput;
exports.isLogGroupingEnabled = isLogGroupingEnabled;
exports.shouldGroupBatchOutput = shouldGroupBatchOutput;
 ...
 */
function isStaticOutputStyle(outputStyle) {
 ...
 */
function printsFullTaskOutput(args) {
    return (!!args.verbose ||
 ...
 */
function isLogGroupingEnabled() {
    return (process.env.NX_SKIP_LOG_GROUPING !== 'true' && !!process.env.GITHUB_ACTIONS);
}
/**
 ...
 */
function shouldGroupBatchOutput() {
...
class CLIOutput {
     ...
     */
    writeTaskOutputChunk(chunk, stream = process.stdout) {
        if (chunk.length > 0 &&
    ...
    logCommandOutput(message, taskStatus, output) {
        ...
        if (grouped) {
            ...
            this.writeToStream(`${GH_GROUP_SUFFIX}${os_1.EOL}`);
        }
    }
    /**
     ...
     */
    logCommandSummary(message, taskStatus) {
        ...
        const command = this.addTaskStatus(taskStatus, this.formatCommand(this.normalizeMessage(message)));
        this.writeToStream(`${icon}  ${command}${os_1.EOL}`);
    }
    /**
    ...
    taskStatus, note) {
        ...
        this.writeToStream(`${icon}  ${command}  ${pc.dim(note)}${os_1.EOL}`);
    }
    /**
     ...
     */
    logBatchGroup(label, body, taskStatus) {
        const grouped = isLogGroupingEnabled();
        ...
        try {
            ...
            if (body.trailer) {
                this.ensureLineStart();
                this.writeToStream(`${body.trailer}${os_1.EOL}`);
            }
        }
        finally {
    ...
    copyFileToStream(path) {
        ...
        try {
            ...
            let bytesRead;
            while ((bytesRead = (0, fs_1.readSync)(fd, buffer, 0, buffer.length, null)) > 0) {
