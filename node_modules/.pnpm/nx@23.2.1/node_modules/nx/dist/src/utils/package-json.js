...
function readNxMigrateConfig(json) {
    ...
    const assertContained = (migrations) => {
        if (!(0, path_2.isContainedRelativePath)(migrations)) {
            throw new Error(`Invalid migrations path "${migrations}" in package "${json.name ?? 'unknown'}@${json.version ?? 'unknown'}": migration paths must not be absolute or escape their base directory through parent traversal.`);
        }
        return migrations;
    };
