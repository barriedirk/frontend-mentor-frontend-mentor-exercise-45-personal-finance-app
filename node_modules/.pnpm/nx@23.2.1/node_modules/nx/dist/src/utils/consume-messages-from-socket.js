...
exports.MessageFramingError = exports.DEFAULT_MAX_MESSAGE_SIZE = exports.MESSAGE_HEADER_PREFIX = void 0;
...
exports.MESSAGE_HEADER_PREFIX = 'NX_MSG_';
const MESSAGE_HEADER_TERMINATOR = ':'.charCodeAt(0);
const MESSAGE_HEADER_PREFIX_BYTES = Buffer.from(exports.MESSAGE_HEADER_PREFIX, 'ascii');
...
const MAX_HEADER_LENGTH = exports.MESSAGE_HEADER_PREFIX.length + 16 + 1;
/**
 ...
 */
exports.DEFAULT_MAX_MESSAGE_SIZE = 2 * 1024 * 1024 * 1024;
function getMaxMessageSize() {
    const configured = process.env.NX_MAX_MESSAGE_SIZE;
    if (configured === undefined || configured === '') {
        return exports.DEFAULT_MAX_MESSAGE_SIZE;
    }
    const parsed = Number(configured);
    if (!Number.isFinite(parsed) || parsed < 0) {
        console.warn(`NX_MAX_MESSAGE_SIZE must be a non-negative number of bytes, but was "${configured}". Using the default of ${exports.DEFAULT_MAX_MESSAGE_SIZE}.`);
        return exports.DEFAULT_MAX_MESSAGE_SIZE;
    }
    return parsed;
}
 ...
 */
function daemonHoldsItsOwnLimit() {
    try {
...
function frameHeader(payloadLength) {
    return Buffer.from(`${exports.MESSAGE_HEADER_PREFIX}${payloadLength}:`, 'ascii');
}
/**
 ...
 */
function writeMessage(socket, payload, callback) {
    socket.write(frameHeader(payload.length));
    socket.write(payload, callback);
}
class MessageFramingError extends Error {
    constructor(message) {
        super(message);
...
exports.MessageFramingError = MessageFramingError;
function consumeMessagesFromSocket(callback, onError = (error) => {
    ...
    const readHeader = () => {
        ...
        for (let i = 0; i < MESSAGE_HEADER_PREFIX_BYTES.length; i++) {
            ...
            if (head[i] !== MESSAGE_HEADER_PREFIX_BYTES[i]) {
                fail(`Expected a message to begin with '${exports.MESSAGE_HEADER_PREFIX}' but received ` +
        ...
        for (let i = MESSAGE_HEADER_PREFIX_BYTES.length; i < head.length; i++) {
            ...
            if (byte === MESSAGE_HEADER_TERMINATOR) {
                ...
                if (maxMessageSize > 0 && value > maxMessageSize) {
                    fail(`Message of ${value} bytes exceeds the ${maxMessageSize} byte limit. ` +
            ...
            if (byte < ZERO || byte > NINE) {
                fail(`Message header contained a non-digit length. The stream is out of sync.`);
                return null;
            }
    ...
    return (data) => {
        ...
        while (!broken) {
            ...
            if (buffered < expectedPayloadLength)
                break;
 ...
 */
function trimToCharBoundary(window, from) {
 ...
 */
function describeMessage(message, { maxBytes = 300, from = 'start' } = {}) {
    const json = isJsonMessage(message);
    const window = from === 'end'
