...
exports.resetSharedRootCacheForTesting = resetSharedRootCacheForTesting;
exports.resolveSharedDataLocation = resolveSharedDataLocation;
exports.sharedUserDataDir = sharedUserDataDir;
exports.sharedDataDirectory = sharedDataDirectory;
...
const nx_tmp_dir_1 = require("./nx-tmp-dir");
const owned_private_dir_1 = require("./owned-private-dir");
const workspace_id_1 = require("./workspace-id");
...
const SHARED_DATA_KINDS = Object.keys(SHARED_SEGMENT);
/**
 ...
 */
function homeDirIsDistinctFromSharedTmp() {
    if (!nx_tmp_dir_1.NX_HOME_TMP_DIR) {
        return false;
    }
    const home = (0, owned_private_dir_1.canonicalDir)(nx_tmp_dir_1.NX_HOME_TMP_DIR);
    return ![nx_tmp_dir_1.NX_TMP_DIR, nx_tmp_dir_1.NX_USER_TMP_DIR].some((shared) => (0, owned_private_dir_1.canonicalDir)(shared) === home);
}
/** Memoizes the verdict below, per repository. */
const sharedRootUsable = new Map();
/** The establish verdict is cached per process; tests need it cleared. */
function resetSharedRootCacheForTesting() {
 ...
 */
function establishUserRoot(workspaceId) {
    if (!nx_tmp_dir_1.NX_HOME_TMP_DIR || !homeDirIsDistinctFromSharedTmp()) {
        return { kind: 'not-a-directory', dir: nx_tmp_dir_1.NX_HOME_TMP_DIR ?? '~/.nx' };
    }
    const repoRoot = (0, path_1.join)(nx_tmp_dir_1.NX_HOME_TMP_DIR, sharedDirName(workspaceId));
    const required = [
        nx_tmp_dir_1.NX_HOME_TMP_DIR,
        repoRoot,
        ...SHARED_DATA_KINDS.map((kind) => (0, path_1.join)(repoRoot, SHARED_SEGMENT[kind])),
    ];
    for (const dir of required) {
        ...
        const result = (0, owned_private_dir_1.ensureOwnedPrivateDir)(dir);
    ...
    for (const kind of SHARED_DATA_KINDS) {
        const refusal = probeWritable((0, path_1.join)(repoRoot, SHARED_SEGMENT[kind]));
        if (refusal) {
            return refusal;
        }
    }
...
function probeWritable(dir) {
    const marker = (0, path_1.join)(dir, `.nx-write-probe-${(0, crypto_1.randomBytes)(8).toString('hex')}`);
    try {
        (0, fs_1.writeFileSync)(marker, '', { flag: 'wx' });
        return undefined;
    }
    catch (e) {
...
function shareableLocation(workspaceId) {
    const cached = sharedRootUsable.get(workspaceId);
    if (cached !== undefined) {
        return cached;
    }
    const location = establishUserRoot(workspaceId)
        ? { share: 'none' }
 ...
 */
function sharedDirName(workspaceId) {
...
function workspaceIdFor(root) {
    if (!workspaceIds.has(root)) {
        ...
        try {
            ...
            // failures.
            id = (0, workspace_id_1.generateWorkspaceId)(root, (0, nx_json_1.readNxJson)(root));
        }
        catch { }
 ...
 */
function resolveSharedDataLocation(root) {
...
function computeSharedDataLocation(root) {
 ...
 */
function sharedUserDataDir(dirName, kind) {
    return (0, path_1.join)(nx_tmp_dir_1.NX_HOME_TMP_DIR, dirName, SHARED_SEGMENT[kind]);
}
/**
 ...
 */
function sharedDataDirectory(root, kind) {
 ...
 */
exports.cacheDir = sharedDataDirectory(workspace_root_1.workspaceRoot, 'cache');
