 ...
 */
function streamChunk(chunk, streamOutput, target = process.stdout) {
    if (streamOutput) {
        output_1.output.writeTaskOutputChunk(chunk, target);
    }
}
/**
 ...
 */
function commandExitedNonZeroWarning(command) {
    ...
    async run() {
        if (this.readyWhenStatus.length) {
            ...
            if (hasFailure && failureDetails) {
                // Add failure message
                const warning = commandExitedNonZeroWarning(failureDetails.childProcess.command);
                terminalOutput += warning;
                streamChunk(warning, this.streamOutput, process.stderr);
...
class RunningNodeProcess {
    ...
    stream(chunk, target = process.stdout) {
