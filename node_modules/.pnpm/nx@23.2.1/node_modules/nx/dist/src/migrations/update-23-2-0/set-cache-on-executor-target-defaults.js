...
async function update(tree) {
    ...
    const collect = (targetName, target) => {
    ...
    for (const [projectName, project] of projects) {
        ...
        for (const [targetName, target] of Object.entries(packageJsonTargets(tree, project.root))) {
            collect(targetName, target);
        }
    }
    let changed = false;
 ...
 */
function canEnableCache(targetDefaults, key, targets, workspaceRoot, projectsMap) {
 ...
 */
function declaresAnyCache(value) {
    if (!value) {
        return false;
    }
 ...
 */
function declaresAnyContinuous(value) {
    if (!value) {
        return false;
    }
 ...
 */
function packageJsonTargets(tree, projectRoot) {
    const packageJsonPath = (0, path_1.joinPathFragments)(projectRoot, 'package.json');
    ...
    const targets = (0, json_1.readJson)(tree, packageJsonPath)?.nx?.targets;
 ...
 */
function executorDeclaresContinuous(executor, workspaceRoot, projectsMap) {
    try {
        const [nodeModule, name] = (0, executor_utils_1.parseExecutor)(executor);
        const { schema } = (0, executor_utils_1.getExecutorInformation)(nodeModule, name, workspaceRoot, projectsMap);
 ...
 */
function isLongRunningTargetName(targetName) {
    return (targetName.endsWith(':watch') ||
