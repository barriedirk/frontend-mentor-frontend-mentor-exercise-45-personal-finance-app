...
exports.findMatchingTargetNames = findMatchingTargetNames;
...
deferSpreadsWithoutBase = true) {
    ...
    if (project.targets) {
        ...
        for (const targetName in project.targets) {
            ...
            let matchingTargets = findMatchingTargetNames(targetName, Object.keys(updatedProjectConfiguration.targets));
 ...
 */
function findMatchingTargetNames(pattern, targetNames) {
    if (!(0, globs_1.isGlobPattern)(pattern)) {
        return [];
    }
    return targetNames.filter((name) => (0, minimatch_1.minimatch)(name, pattern));
}
function readProjectConfigurationsFromRootMap(projectRootMap) {
    ...
    if (conflicts.size > 0) {
        ...
        throw new error_types_1.MultipleProjectsWithSameNameError(conflicts, projects, (0, git_worktrees_1.analyzeWorktreeConflicts)(workspace_root_1.workspaceRoot, conflicts) ?? undefined);
    }
