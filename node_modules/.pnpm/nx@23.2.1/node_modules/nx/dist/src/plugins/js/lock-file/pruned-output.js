...
exports.LOCAL_PATH_MODULES_DIR = exports.PNPM_LOCKFILE_RESOLUTION_CONFIG_FIELDS = void 0;
exports.stripPrunedLockfilePnpmConfig = stripPrunedLockfilePnpmConfig;
exports.dropInheritedPnpmPatchedDependencies = dropInheritedPnpmPatchedDependencies;
exports.getPrunedPnpmInstallSettingsYaml = getPrunedPnpmInstallSettingsYaml;
exports.normalizePrunedPatchPath = normalizePrunedPatchPath;
exports.getPrunedPnpmPackageJsonBuildSettings = getPrunedPnpmPackageJsonBuildSettings;
exports.getPrunedPnpmPatchArtifacts = getPrunedPnpmPatchArtifacts;
exports.containLocalPath = containLocalPath;
exports.uncontainLocalPath = uncontainLocalPath;
exports.normalizeLocalPathSpec = normalizeLocalPathSpec;
exports.uncontainLocalPathSpec = uncontainLocalPathSpec;
exports.warnOnWorkspaceModulePathCollision = warnOnWorkspaceModulePathCollision;
exports.containShippedLocalFilePaths = containShippedLocalFilePaths;
exports.containShippedLocalLinkRefs = containShippedLocalLinkRefs;
exports.getPrunedPnpmLocalPathArtifacts = getPrunedPnpmLocalPathArtifacts;
exports.validatePrunedLocalPathClosure = validatePrunedLocalPathClosure;
exports.relocatePrunedLocalPathSpec = relocatePrunedLocalPathSpec;
exports.warnIncompletePrunedPnpmOutput = warnIncompletePrunedPnpmOutput;
exports.warnUnshippableLocalPathSpec = warnUnshippableLocalPathSpec;
exports.movePeerDependencyToDependencies = movePeerDependencyToDependencies;
exports.dropEmptyPeerDependencySections = dropEmptyPeerDependencySections;
exports.rewritePrunedLocalPathSpecifiers = rewritePrunedLocalPathSpecifiers;
exports.getPrunedPnpmInstallArtifacts = getPrunedPnpmInstallArtifacts;
...
const PNPM_MANIFEST_RESOLUTION_CONFIG_FIELDS = PNPM_RESOLUTION_CONFIG.flatMap((field) => (field.manifest === null ? [] : [field.manifest]));
exports.PNPM_LOCKFILE_RESOLUTION_CONFIG_FIELDS = PNPM_RESOLUTION_CONFIG.map((field) => field.lockfile);
/**
 ...
 */
function stripPrunedLockfilePnpmConfig(packageJson) {
    if (!packageJson.pnpm) {
        return;
    }
 ...
 * trusted here either.
 */
function dropInheritedPnpmPatchedDependencies(packageJson) {
 ...
 */
function getPrunedPnpmInstallSettingsYaml(workspaceRootPath = workspace_root_1.workspaceRoot, prunedLockfileContent, precomputed) {
    const settings = getPrunedPnpmWorkspaceSettings(workspaceRootPath, prunedLockfileContent, precomputed) ?? {};
 ...
 *   accepted.
 */
const STRAIGHT_COPY_WORKSPACE_SETTINGS = [
    'supportedArchitectures',
    'minimumReleaseAge',
 ...
 */
function getPrunedPnpmWorkspaceSettings(workspaceRootPath, prunedLockfileContent, precomputed) {
    ...
    let rootSettings;
    try {
        const rootWorkspaceYaml = (0, path_1.join)(workspaceRootPath, 'pnpm-workspace.yaml');
        if (!(0, fs_1.existsSync)(rootWorkspaceYaml)) {
            return null;
        }
        // An empty or comment-only file parses to null/undefined; treat it as no
        // settings rather than dereferencing it below.
        rootSettings = (0, fileutils_1.readYamlFile)(rootWorkspaceYaml) ?? {};
    }
    catch {
        // Unreadable or malformed pnpm-workspace.yaml: skip rather than guess.
        logger_1.logger.warn('Could not read the workspace root pnpm-workspace.yaml; the pruned output will not declare pnpm install settings (build-script approvals, supportedArchitectures, minimumReleaseAge, patchedDependencies).');
        return null;
    }
    const settings = {};
    if (rootSettings.allowBuilds) {
        const allowBuilds = prunedLockfileContent
            ? filterAllowBuildsToLockfile(rootSettings.allowBuilds, prunedLockfileContent)
    ...
    const patchedDependencies = precomputed?.patchedDependencies ??
...
const pnpmMajorByWorkspaceRoot = new Map();
/**
 ...
 * it on every rebuild.
 */
function getPnpmMajor(workspaceRootPath) {
    if (pnpmMajorByWorkspaceRoot.has(workspaceRootPath)) {
        return pnpmMajorByWorkspaceRoot.get(workspaceRootPath);
    }
    let major;
    try {
        const parsed = Number.parseInt((0, package_manager_1.getPackageManagerVersion)('pnpm', workspaceRootPath).split('.')[0], 10);
 ...
 */
function getPnpmMajorOrWarn(workspaceRootPath) {
    const pnpmMajor = getPnpmMajor(workspaceRootPath);
    if (pnpmMajor === null) {
        logger_1.logger.warn('Could not determine the pnpm version. The pruned output will not carry pnpm build-script approvals, supportedArchitectures, minimumReleaseAge, or patchedDependencies declarations; patch files still ship.');
    }
    return pnpmMajor;
}
/**
 ...
 */
function resolvePnpmMajor(precomputed, workspaceRootPath) {
 ...
 */
function filterAllowBuildsToLockfile(allowBuilds, prunedLockfileContent) {
...
let lastParsedPnpmLockfile = null;
/**
 ...
 * returned document.
 */
function parsePnpmLockfileYaml(content) {
    if (lastParsedPnpmLockfile?.content !== content) {
        ...
        // the process.
        const warnUnusable = (cause) => {
            logger_1.logger.warn(`Could not parse the pnpm lockfile (${cause}); the pruned output will not carry patch declarations, local-path artifacts, or a validated link closure.`);
        };
        let parsed;
        ...
        const document = (0, pnpm_normalizer_1.extractMainLockfileDocument)(content);
        ...
        lastParsedPnpmLockfile = { content, parsed: parsed };
    }
 ...
 */
function getBuildApprovalScopeNames(lockfileContent) {
    const parsed = parsePnpmLockfileYaml(lockfileContent);
    ...
    if (Object.keys(parsed.importers ?? {}).some((importer) => importer !== '.')) {
        return null;
    }
    const names = new Set();
 ...
 */
function getPrunedPatchedDependencies(workspaceRootPath, prunedLockfileContent) {
 ...
 */
function normalizePrunedPatchPath(patchPath) {
    const segments = path_1.posix
        .normalize(patchPath.replace(/\\/g, '/'))
        .split('/')
        .filter((segment) => segment !== '' && segment !== '.' && segment !== '..');
    return `patches/${segments.join('/')}`;
}
function normalizePrunedPatchedDependencies(patchedDependencies) {
    const normalized = {};
...
function readRootPatchedDependencies(workspaceRootPath) {
    const merged = {};
    ...
    try {
        const rootPackageJsonPath = (0, path_1.join)(workspaceRootPath, 'package.json');
        if ((0, fs_1.existsSync)(rootPackageJsonPath)) {
            const rootPackageJson = (0, fileutils_1.readJsonFile)(rootPackageJsonPath);
            Object.assign(merged, rootPackageJson.pnpm?.patchedDependencies ?? {});
        }
    }
    catch {
        logger_1.logger.warn('Could not read patchedDependencies from the workspace root package.json; the pruned output will not carry the patches it declares.');
    }
    try {
        const rootWorkspaceYaml = (0, path_1.join)(workspaceRootPath, 'pnpm-workspace.yaml');
        if ((0, fs_1.existsSync)(rootWorkspaceYaml)) {
            const yaml = (0, fileutils_1.readYamlFile)(rootWorkspaceYaml);
            Object.assign(merged, yaml?.patchedDependencies ?? {});
        }
    }
    catch {
        logger_1.logger.warn('Could not read patchedDependencies from the workspace root pnpm-workspace.yaml; the pruned output will not carry the patches it declares.');
    }
    return merged;
}
/** Copies `key` from `source` to `target` unless the source leaves it unset. */
function assignDefined(target, source, key) {
 ...
 */
function readRootPnpmBuildSettings(workspaceRootPath) {
    const fields = [
    ...
    try {
        const rootPackageJsonPath = (0, path_1.join)(workspaceRootPath, 'package.json');
        if ((0, fs_1.existsSync)(rootPackageJsonPath)) {
            sources.push((0, fileutils_1.readJsonFile)(rootPackageJsonPath).pnpm ?? {});
        }
    }
    catch {
        logger_1.logger.warn('Could not read the pnpm build-script settings from the workspace root package.json; the pruned output will not carry the settings it declares.');
    }
    try {
        const rootWorkspaceYaml = (0, path_1.join)(workspaceRootPath, 'pnpm-workspace.yaml');
        if ((0, fs_1.existsSync)(rootWorkspaceYaml)) {
            sources.push((0, fileutils_1.readYamlFile)(rootWorkspaceYaml) ?? {});
        }
    }
    catch {
        logger_1.logger.warn('Could not read the pnpm build-script settings from the workspace root pnpm-workspace.yaml; the pruned output will not carry the settings it declares.');
    }
 ...
 */
function getPrunedPnpmPackageJsonBuildSettings(workspaceRootPath = workspace_root_1.workspaceRoot, prunedLockfileContent, precomputed) {
    const pnpmMajor = resolvePnpmMajor(precomputed, workspaceRootPath);
    if (pnpmMajor === null || pnpmMajor >= 11) {
        return null;
    }
 ...
 */
function applyPrunedPnpmPackageJsonSettings(packageJson, buildSettings, patchedDependencies) {
    ...
    if (buildSettings?.onlyBuiltDependencies) {
        packageJson.pnpm.onlyBuiltDependencies = union(packageJson.pnpm.onlyBuiltDependencies, buildSettings.onlyBuiltDependencies);
    }
    if (buildSettings?.neverBuiltDependencies) {
        packageJson.pnpm.neverBuiltDependencies = union(packageJson.pnpm.neverBuiltDependencies, buildSettings.neverBuiltDependencies);
    }
...
function getPrunedLockfilePatchedKeys(prunedLockfileContent) {
    const parsed = parsePnpmLockfileYaml(prunedLockfileContent);
 ...
 */
function getPrunedPnpmPatchArtifacts(workspaceRootPath = workspace_root_1.workspaceRoot, prunedLockfileContent, precomputed) {
    const patchedDependencies = precomputed?.patchedDependencies ??
        getPrunedPatchedDependencies(workspaceRootPath, prunedLockfileContent);
    if (Object.keys(patchedDependencies).length === 0) {
    ...
    for (const patchPath of new Set(Object.values(patchedDependencies))) {
        ...
        const source = (0, path_1.isAbsolute)(patchPath)
            ? patchPath
            : (0, path_1.join)(workspaceRootPath, patchPath);
        ...
        if (existingSource !== undefined && existingSource !== source) {
            throw new Error(`Cannot prune pnpm patches: "${existingSource}" and "${source}" both ship to "${destination}" in the standalone output. Rename one so the patches do not collide.`);
        }
        ...
        if ((0, fileutils_1.fileExists)(source)) {
            ...
            logger_1.logger.warn(`Patch file "${patchPath}" referenced by patchedDependencies was not found; the pruned output declares the patch but cannot ship the file.`);
        }
    }
    const pnpmMajor = resolvePnpmMajor(precomputed, workspaceRootPath);
    return {
        patchFiles,
        packageJsonPatchedDependencies: pnpmMajor !== null && pnpmMajor < 11
            ? normalizePrunedPatchedDependencies(patchedDependencies)
            : null,
    };
}
 ...
 */
exports.LOCAL_PATH_MODULES_DIR = 'local_path_modules';
/**
 ...
 */
function containLocalPath(wsRelativePath) {
    return `${exports.LOCAL_PATH_MODULES_DIR}/${wsRelativePath}`;
}
/**
 ...
 */
function uncontainLocalPath(shippedPath) {
    if (shippedPath === exports.LOCAL_PATH_MODULES_DIR) {
        return '';
    }
    return shippedPath.startsWith(`${exports.LOCAL_PATH_MODULES_DIR}/`)
        ? shippedPath.slice(exports.LOCAL_PATH_MODULES_DIR.length + 1)
        : shippedPath;
}
/**
 ...
 */
function normalizeLocalPathSpec(spec) {
    const protocol = spec.startsWith('link:')
        ...
        : spec.startsWith('file:')
            ...
            : null;
    return protocol
        ? `${protocol}${(0, path_2.normalizePath)(spec.slice(protocol.length))}`
        : spec;
}
/**
 ...
 */
function uncontainLocalPathSpec(spec) {
 ...
 */
function resolveLocalPathSource(wsRelativePath, workspaceRootPath, workspaceRootRealPath) {
    if (localPathEscapesOutput(wsRelativePath)) {
        return { source: null, reason: 'outside-workspace' };
    }
    const attempted = (0, path_1.join)(workspaceRootPath, wsRelativePath);
    let resolved;
    try {
        resolved = (0, fs_1.realpathSync)(attempted);
    }
    catch {
    ...
    if (!resolved.startsWith(`${workspaceRootRealPath}${path_1.sep}`)) {
 ...
 */
function containVendoredFilePath(wsRelativePath, synthesized) {
    if (wsRelativePath === '' ||
        wsRelativePath === '.' ||
 ...
 */
function containFileToken(token, synthesized) {
    ...
    const path = (0, path_2.normalizePath)(token.slice(pathStart));
 ...
 */
function warnOnWorkspaceModulePathCollision(sourcePackages, synthesizedModulePaths) {
    for (const snapshot of Object.values(sourcePackages ?? {})) {
        const directory = snapshot
            ?.resolution?.directory;
        if (typeof directory === 'string' &&
            synthesizedModulePaths.has((0, path_2.normalizePath)(directory))) {
            logger_1.logger.warn(`Local-path dependency "file:${directory}" sits where the pruned output copies a workspace module of the same name, so it cannot ship separately. Move it out of ${WORKSPACE_MODULES_DIR}/ to deploy it.`);
        }
    }
}
/**
 ...
 * old path.
 */
function containShippedLocalFilePaths(lockfile, synthesizedModulePaths = new Set()) {
    const synthesized = synthesizedModulePaths;
    const containSnapshot = (snapshot) => {
        ...
        if (resolution && typeof resolution === 'object') {
            if (typeof resolution.directory === 'string') {
                ...
                resolution.directory = containVendoredFilePath((0, path_2.normalizePath)(resolution.directory), synthesized);
            }
 ...
 */
function containShippedLocalLinkRefs(sourceSnapshots) {
...
function localPathEscapesOutput(wsRelativePath) {
    ...
    return ((0, path_1.isAbsolute)(wsRelativePath) || wsRelativePath.split(/[\\/]/).includes('..'));
}
/** True when a resolution `directory` points at a copied workspace module. */
function isUnderWorkspaceModules(directory) {
    return (directory === WORKSPACE_MODULES_DIR ||
 ...
 */
function resolveLinkTarget(linkRef) {
    const refPath = linkRef.slice('link:'.length);
    return (0, path_2.normalizePath)((0, path_1.isAbsolute)(refPath) ? refPath : (0, path_1.join)(refPath));
}
function parsePrunedLockfile(content) {
    return parsePnpmLockfileYaml(content);
}
/**
...
function collectPrunedLinkTargetDirs(parsed) {
    ...
    const addRef = (value) => {
        ...
        if (target === '' || target === '.') {
            logger_1.logger.warn(`Local-path dependency "${ref}" resolves to the workspace root itself and cannot be shipped into the pruned output.`);
            return;
        }
 ...
 */
function getPrunedPnpmLocalPathArtifacts(workspaceRootPath = workspace_root_1.workspaceRoot, prunedLockfileContent) {
    if (!prunedLockfileContent) {
        return [];
    }
    ...
    try {
        workspaceRootRealPath = (0, fs_1.realpathSync)(workspaceRootPath);
    }
    catch {
    ...
    const resolveShippableSource = (wsRelativePath, origin) => {
        const resolution = resolveLocalPathSource(wsRelativePath, workspaceRootPath, workspaceRootRealPath);
        if (resolution.source) {
            return resolution.source;
        }
        if (resolution.reason === 'missing') {
            logger_1.logger.warn(`Local-path dependency "${origin}" was not found at ${resolution.attempted}; the pruned output references it but cannot ship it.`);
        }
        else if (resolution.reason === 'workspace-root') {
            warnUnshippableLocalPathSpec(`"${origin}"`, 'workspace-root');
        }
        else {
            logger_1.logger.warn(resolution.resolved
    ...
    const shipFile = (shippedPath, origin) => {
        if (shippedRoots.has(shippedPath)) {
            return;
        }
        shippedRoots.add(shippedPath);
        const source = resolveShippableSource(uncontainLocalPath(shippedPath), origin);
    ...
    const shipDirectory = (shippedPath, origin) => {
        if (shippedRoots.has(shippedPath)) {
            return;
        }
        shippedRoots.add(shippedPath);
        ...
        if (!(0, fs_1.statSync)(source).isDirectory()) {
            logger_1.logger.warn(`Local-path dependency "${origin}" is not a directory at ${source}; the pruned output references it but cannot ship it.`);
            return;
        }
        // Walk the tree, skipping node_modules, deduping by destination.
        const walk = (absoluteDir, destinationDir) => {
            for (const entry of (0, fs_1.readdirSync)(absoluteDir, { withFileTypes: true })) {
                if (entry.name === 'node_modules') {
                    continue;
                }
                const absoluteEntry = (0, path_1.join)(absoluteDir, entry.name);
                ...
                if (entry.isDirectory()) {
                    ...
                    logger_1.logger.warn(`Local-path dependency "${origin}" contains a symbolic link at ${absoluteEntry}, which is not shipped into the pruned output.`);
                }
            }
        };
    ...
    for (const snapshot of Object.values(parsed.packages ?? {})) {
        ...
        if (directory && !isUnderWorkspaceModules(directory)) {
            shipDirectory((0, path_2.normalizePath)(directory), `file:${directory}`);
        }
    }
 ...
 */
function prunedLockfileReferencesLocalPaths(prunedLockfileContent) {
    const parsed = parsePrunedLockfile(prunedLockfileContent);
    if (!parsed) {
        return false;
    }
 ...
 */
function validatePrunedLocalPathClosure(packageJson, workspaceRootPath, prunedLockfileContent) {
    if (!prunedLockfileContent) {
        return;
    }
    const parsed = parsePrunedLockfile(prunedLockfileContent);
    ...
    for (const [key, entry] of Object.entries(parsed.packages ?? {})) {
        ...
        const source = uncontainLocalPath((0, path_2.normalizePath)(directory)).replace(/\/+$/, '');
        ...
        const hasEdges = LOCKFILE_DEP_SECTIONS.some((section) => Object.keys(snapshot?.[section] ?? {}).length > 0);
        if (!hasEdges) {
    ...
    for (const [target, kind] of targets) {
        ...
        if (!resolveLocalPathSource(target, workspaceRootPath, workspaceRootRealPath)
            .source) {
            continue;
        }
        const manifestPath = (0, path_1.join)(workspaceRootPath, target, 'package.json');
        ...
        let targetManifest;
        try {
            targetManifest = (0, fileutils_1.readJsonFile)(manifestPath);
        }
        catch {
            logger_1.logger.warn(`${descriptor} ${target} has an unreadable package.json at ${manifestPath}; its dependency closure was not validated for the standalone deploy.`);
            continue;
        }
        ...
        for (const dep of Object.keys(targetManifest.dependencies ?? {})) {
            ...
            if (rootDev.has(dep)) {
                logger_1.logger.warn(`${descriptor} ${targetName} requires ${dep}, which is only a devDependency of ${appName}; a production (--prod) install of the standalone deploy would omit it.`);
                continue;
            }
            throw new Error(kind === 'link'
        ...
        for (const dep of [
            ...
            logger_1.logger.warn(`${descriptor} ${targetName} may need ${dep} at runtime, which is not a dependency of ${appName}; add it to ${appName} if it fails to resolve.`);
        }
    }
}
/**
 ...
 * cannot disagree.
 */
function relocatePrunedLocalPathSpec(spec, sourceDir, destDir) {
    const protocol = spec.startsWith('link:')
        ? 'link:'
        : spec.startsWith('file:')
            ? 'file:'
    ...
    const rawPath = (0, path_2.normalizePath)(spec.slice(protocol.length));
    ...
    if ((0, path_1.isAbsolute)(rawPath)) {
    ...
    const wsRelativeTarget = (0, path_2.normalizePath)((0, path_1.join)(sourceDir, rawPath)).replace(/\/+$/, '');
    ...
    if ((0, path_1.isAbsolute)(wsRelativeTarget) ||
    ...
    const relocated = destDir === '' || destDir === '.'
        ? shippedTarget
        : (0, path_2.normalizePath)((0, path_1.relative)(destDir, shippedTarget));
 ...
 */
function warnIncompletePrunedPnpmOutput(lockFileContent, workspaceRootPath = workspace_root_1.workspaceRoot) {
    const missing = [];
    // Keyed on the approvals themselves rather than on the emitted file, which a
    // workspace declaring only patches still gets.
    const workspaceSettings = getPrunedPnpmWorkspaceSettings(workspaceRootPath, lockFileContent);
    if (workspaceSettings?.allowBuilds !== undefined ||
        ...
        missing.push('the build-script approvals and supportedArchitectures the workspace declares');
    }
    if (Object.keys(getPrunedPatchedDependencies(workspaceRootPath, lockFileContent)).length > 0) {
    ...
    output_1.output.warn({
        ...
        bodyLines: [
            `A standalone install of the output will be missing ${missing.join(', and ')}.`,
            'Use generatePrunedDeployOutput to ship them.',
        ],
    });
}
/** Warns that a local-path target cannot ship, with the reason-specific remedy. */
function warnUnshippableLocalPathSpec(description, reason) {
    logger_1.logger.warn(reason === 'workspace-root'
 ...
 */
function movePeerDependencyToDependencies(packageJson, name, spec) {
 ...
 */
function dropEmptyPeerDependencySections(packageJson) {
    if (packageJson.peerDependencies &&
 ...
 */
function rewritePrunedLocalPathSpecifiers(packageJson, projectRoot, workspaceRootPath, workspacePackageNames) {
    const catalogManager = (0, catalog_1.getCatalogManager)(workspaceRootPath);
    const sections = [
        'dependencies',
        'optionalDependencies',
        'devDependencies',
    ...
    for (const section of sections) {
        ...
        for (const [name, specifier] of Object.entries(deps)) {
            ...
            if (catalogManager?.isCatalogReference(specifier)) {
                const resolvedCatalog = catalogManager.resolveCatalogReference(workspaceRootPath, name, specifier);
                if (resolvedCatalog) {
            ...
            if (relocation.reason) {
                warnUnshippableLocalPathSpec(`"${name}": "${resolved}"`, relocation.reason);
            }
 ...
 */
function getPrunedPnpmInstallArtifacts(workspaceRootPath, prunedLockfileContent, packageJson, options) {
    const config = {
        pnpmMajor: getPnpmMajorOrWarn(workspaceRootPath),
        patchedDependencies: getPrunedPatchedDependencies(workspaceRootPath, prunedLockfileContent),
    };
    const { patchFiles, packageJsonPatchedDependencies } = getPrunedPnpmPatchArtifacts(workspaceRootPath, prunedLockfileContent, config);
    const yaml = getPrunedPnpmInstallSettingsYaml(workspaceRootPath, prunedLockfileContent, config);
    const artifacts = [
        { path: 'pnpm-workspace.yaml', content: yaml },
    ];
    artifacts.push(...patchFiles);
    if (options?.includeLocalPathArtifacts !== false) {
        artifacts.push(...getPrunedPnpmLocalPathArtifacts(workspaceRootPath, prunedLockfileContent));
    }
    const buildSettings = getPrunedPnpmPackageJsonBuildSettings(workspaceRootPath, prunedLockfileContent, config);
    applyPrunedPnpmPackageJsonSettings(packageJson, buildSettings, packageJsonPatchedDependencies);
    return artifacts;
}
