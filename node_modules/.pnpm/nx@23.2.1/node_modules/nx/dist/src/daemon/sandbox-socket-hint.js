...
exports.sandboxSocketHint = sandboxSocketHint;
 ...
 */
function socketRoots() {
    const configured = process.env.NX_SOCKET_DIR || process.env.NX_DAEMON_SOCKET_DIR;
    if (configured) {
        return configured;
    }
    return [nx_tmp_dir_1.NX_TMP_DIR, nx_tmp_dir_1.NX_HOME_TMP_DIR ? '~/.nx' : undefined]
        .filter(Boolean)
        .join(' or ');
}
/**
 ...
 */
function sandboxSocketHint({ certain = false, } = {}) {
    ...
    return [
        ...
        ...((0, native_1.detectAiAgent)() === 'claude'
            ? [
 ...
 * describe the grant.
 */
function sandboxSpecificRemedy(roots, certain) {
    const sandboxed = (0, is_sandbox_1.isSandbox)();
    const agent = (0, native_1.detectAiAgent)();
    const knownAgent = agent === 'claude' || agent === 'codex' || agent === 'copilot-cli';
