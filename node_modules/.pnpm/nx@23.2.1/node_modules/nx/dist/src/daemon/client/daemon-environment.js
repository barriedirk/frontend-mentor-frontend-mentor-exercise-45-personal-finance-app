...
exports.hashDaemonClientEnv = hashDaemonClientEnv;
exports.getDaemonEnv = getDaemonEnv;
exports.getDaemonClientEnvGeneration = getDaemonClientEnvGeneration;
exports.getDaemonSpawnEnv = getDaemonSpawnEnv;
exports.getAppliedDaemonClientEnv = getAppliedDaemonClientEnv;
exports.applyDaemonEnvFromClient = applyDaemonEnvFromClient;
exports.normalizeDaemonEnvironmentForGraph = normalizeDaemonEnvironmentForGraph;
exports.getChangedEnvKeys = getChangedEnvKeys;
...
function isExcludedEnvVar(key) {
    if (DAEMON_ENV_VARS_EXCLUSIONS.has(key) ||
...
function hashDaemonClientEnv() {
    ...
    return (0, file_hasher_1.hashObject)(env);
}
/**
 ...
 */
function normalizedLoadDotEnvFiles() {
 ...
 */
function getDaemonClientEnvGeneration() {
 ...
 * nothing.
 */
function getAppliedDaemonClientEnv() {
 ...
 */
function normalizeDaemonEnvironmentForGraph(env, platform = process.platform) {
    const normalized = { ...env };
    const berryBinFolder = normalized.BERRY_BIN_FOLDER;
    if (!berryBinFolder) {
        return normalized;
    }
    const pathKey = Object.keys(normalized).find((key) => key.toUpperCase() === 'PATH');
...
function getChangedEnvKeys(before, after) {
    const changedKeys = [];
