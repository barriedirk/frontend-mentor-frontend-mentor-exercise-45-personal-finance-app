...
const handle_client_env_1 = require("./handle-client-env");
...
const handle_outputs_changes_1 = require("./handle-outputs-changes");
...
const server = (0, net_1.createServer)(async (socket) => {
    ...
    socket.on('data', (0, consume_messages_from_socket_1.consumeMessagesFromSocket)(async (message) => {
        ...
        await handleMessage(socket, message).catch(async (e) => {
            await (0, shutdown_utils_1.respondWithError)(socket, 'Error handling message', e);
        });
    }, (err) => {
        logger_1.serverLogger.log(`Framing error: ${err.message}`);
        // The stream cannot resynchronize, so close it and let the client
        // observe the disconnect instead of waiting on a reply.
        (0, shutdown_utils_1.respondWithError)(socket, 'Malformed message', err).finally(() => socket.destroy());
    }));
    socket.on('error', (e) => {
...
async function handleMessage(socket, data) {
    ...
    const outputsWatcherTerminalError = (0, handle_outputs_changes_1.getOutputsWatcherTerminalError)();
    if (outputsWatcherTerminalError) {
        await (0, shutdown_utils_1.respondWithErrorAndExit)(socket, `File watcher error in the workspace '${workspace_root_1.workspaceRoot}'.`, outputsWatcherTerminalError);
    }
    (0, shutdown_utils_1.resetInactivityTimeout)(handleInactivityTimeout);
    const unparsedPayload = data;
    let payload;
    // Reply in the format the client used.
    const mode = (0, consume_messages_from_socket_1.isJsonMessage)(unparsedPayload) ? 'json' : 'v8';
    logger_1.serverLogger.log(`Received raw message of length ${unparsedPayload.length}`);
    try {
        payload = (0, consume_messages_from_socket_1.parseMessage)(unparsedPayload);
    }
    catch (e) {
        await (0, shutdown_utils_1.respondWithErrorAndExit)(socket, `Invalid payload from the client`, new Error(`Unsupported payload sent to daemon server: ${(0, consume_messages_from_socket_1.describeMessage)(unparsedPayload, { maxBytes: 200 })}`));
    }
    ...
    if ((0, daemon_message_1.isDaemonMessage)(payload)) {
        try {
            (0, daemon_message_1.assertNotForeignWorkspaceMessage)(payload, workspace_root_1.workspaceRoot);
        }
        catch (e) {
            await (0, shutdown_utils_1.respondWithError)(socket, `Workspace root mismatch`, e);
            return;
        }
    }
    if ((0, daemon_message_1.isDaemonMessage)(payload) && payload.env) {
        await (0, handle_client_env_1.handleClientEnv)(payload.env);
    }
...
async function handleResult(socket, type, hrFn, mode) {
    ...
    if (hr.error) {
        ...
        const response = typeof hr.response === 'string'
            ...
            : (0, socket_utils_1.serializeWithFallback)(hr.response, mode);
...
const handleWorkspaceChanges = async (err, changeEvents) => {
    ...
    try {
        ...
        if (err) {
            ...
            (0, file_watcher_sockets_1.notifyFileWatcherSocketsOfError)(error);
            return;
        }
        ...
        (0, file_watcher_sockets_1.notifyFileWatcherSocketsOfError)(err);
    }
};
async function startServer() {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        server.on('error', (error) => {
            logger_1.serverLogger.log(`Failed to listen on: ${socketPath} (${error.message})`);
            process.exit((0, permission_errors_1.isPermissionDenied)(error) ? socket_refused_exit_code_1.SOCKET_REFUSED_EXIT_CODE : 1);
        });
        try {
            server.listen(socketPath, async () => {
                try {
                    ...
                    if (!socket_utils_1.isWindows) {
                        try {
                            (0, fs_1.chmodSync)(socketPath, 0o600);
                        }
                        catch {
                    ...
                    if (!(0, shutdown_utils_1.getOutputWatcherInstance)()) {
                        (0, shutdown_utils_1.storeOutputWatcherInstance)(await (0, watcher_1.watchOutputFiles)(server, handle_outputs_changes_1.handleOutputsChanges));
                    }
