...
class StaticRunOneTerminalOutputLifeCycle {
    ...
    endCommand() {
        ...
        if (this.failedTasks.length === 0 && this.stoppedTasks.length === 0) {
            ...
            if (this.failedTasks.length > 0) {
                bodyLines.push(output_1.output.dim('Failed tasks:'), '', ...this.failedTasks.map((task) => `${output_1.output.dim('-')} ${task.id}`), '');
            }
            bodyLines.push(...this.hiddenOutputHint());
            bodyLines.push(`${output_1.output.dim('Hint: run the command with')} --verbose ${output_1.output.dim('for more details.')}`);
    ...
    printTaskTerminalOutput(task, status, terminalOutput) {
         ...
         */
        this.collapsedTasks++;
        output_1.output.logCommandSummary(args.join(' '), status);
    }
}
