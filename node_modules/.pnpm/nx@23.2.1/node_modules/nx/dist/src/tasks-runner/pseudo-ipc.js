...
class PseudoIPCServer {
    ...
    sendMessageToChildren(message) {
        this.sockets.forEach((socket) => {
            (0, socket_utils_1.sendMessage)(socket, { type: 'TO_CHILDREN_FROM_PARENT', message });
        });
    }
    sendMessageToChild(id, message) {
        this.sockets.forEach((socket) => {
            (0, socket_utils_1.sendMessage)(socket, { type: 'TO_CHILDREN_FROM_PARENT', id, message });
        });
    }
...
class PseudoIPCClient {
    ...
    sendMessageToParent(message) {
        (0, socket_utils_1.sendMessage)(this.socket, { type: 'TO_PARENT_FROM_CHILDREN', message });
    }
    notifyChildIsReady(id) {
        (0, socket_utils_1.sendMessage)(this.socket, {
            type: 'CHILD_READY',
            message: id,
        });
    }
