...
const performance_report_1 = require("./life-cycles/performance-report");
...
async function getTerminalOutputLifeCycle(initiatingProject, initiatingTasks, projectNames, tasks, taskGraph, nxArgs, nxJson, overrides) {
    ...
    (0, performance_report_1.prefetchRemoteCacheOnboardingUrl)(nxJson);
...
async function promptForApplyingSyncGeneratorChanges() {
    try {
        ...
        const applyChanges = await (0, prompt_helpers_1.selectPrompt)({
...
async function confirmRunningTasksWithSyncFailures() {
    try {
        const runTasks = await (0, prompt_helpers_1.selectPrompt)({
...
function setEnvVarsBasedOnArgs(nxArgs, loadDotEnvFiles) {
    ...
    if (nxArgs.outputStyle == 'stream' ||
        (batchMode && !(0, output_1.isLogGroupingEnabled)())) {
...
function shouldUseDynamicLifeCycle(tasks, options, outputStyle) {
    ...
    if ((0, is_ci_1.isCI)())
        return false;
    if ((0, output_1.isStaticOutputStyle)(outputStyle) ||
        outputStyle === 'stream' ||
